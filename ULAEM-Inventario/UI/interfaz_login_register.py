...
class LoginRegister:
    ...
    def crear_login(self, frame, navegacion, contenido_principal):
        ...
        # ]ctk.CTkLabel(frame, text=, font=self.fuentes.obtener_fuente_titulos_subtitulos()).
        
        inicio_sesion = self.widget.crear_label_titulo_subtitulos(frame, "Iniciar Sesión")
        inicio_sesion.pack(pady=10)
        
        # campos de entrada y botones
        self.widget.crear_label_titulo_subtitulos(frame, "Cedula").pack(pady=10)
        # ctk.CTkLabel(frame, text="Cedula: ", font=self.fuentes.obtener_fuente_titulos_subtitulos()).pack(pady=10)
        entry_cedula = self.widget.crear_entry(frame, False)
        entry_cedula.pack(pady=10)
        
        # campos de contraseña
        self.widget.crear_label_titulo_subtitulos(frame, "Contraseña").pack(pady=10)
        entry_password = self.widget.crear_entry(frame, True)
        ...
        button_login = self.widget.crear_boton(frame_botones, "Iniciar Sesión", 
                                               lambda: self.validar_y_gestionar("login", navegacion, contenido_principal, entry_cedula, entry_password))
        button_login.grid(row=0, column=0, padx=10)

        button_register = self.widget.crear_boton(frame_botones, "Crear Cuenta aqui",
                                                lambda: self.mostrar_register(navegacion, contenido_principal))
    ...
    def crear_register(self, frame, navegacion, contenido_principal):
        ...
        self.widget.crear_label_titulo_subtitulos(frame, "Registrar Nuevo Usuario").pack(pady=10)

        # Formulario para el registro
        frame_formulario = self.widget.crear_frame(frame)
        ...
        entradas_de_valores = {}
        ############################################## Revisar porque no centra correctamente
        for label_texto, fila in campos:    
            label = self.widget.crear_label_texto_normal(frame_formulario, label_texto)
            label.grid(row=fila, column=0, padx=10, pady=10, sticky="w")
            
            entry = self.widget.crear_entry(frame_formulario, False)
            entry.grid(row=fila, column=1, padx=10, pady=10, sticky="w")
            entradas_de_valores[label_texto] = entry
        #############################################
        
        rango = ["Software", "IT", "Profesor"]
        entry_rango = self.widget.crear_checkbox(frame_formulario, rango)
